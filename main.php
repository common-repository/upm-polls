...
Plugin URI: http://www.profprojects.com/
Description: Best Plugin to create Polls for your site.
Version: 1.0.3
Author: ProfProjects ( Artyom Chakhoyan )
Author URI: http://www.gconverter.com
*/

/*  Copyright 20011  Artyom Chakhoyan by ProfProjects.com (email : tom.webdever@gmail.com , Support@ProfProjects.com)
...
<div class="upm_poll_footer"> Total Voters: [TOTAL-VOTERS] [NEXT-POLL] </div>
	...
	if( get_option('pppm_1_installed') !='1.0.3' ){
		///////////////////////////////////////////////
		$ndt = get_option('pppm_poll_form_template');
		if( strpos( $ndt, '[RESULT]' ) === FALSE ){
			$ndt = preg_replace( '|>[\s\t\n\r]*?\[ANSWER\][\s\t\n\r]*?(</label>)[\s\t\n\r]*?</li>|is',"</label>\r\n[RESULT]", $ndt );
			update_option('pppm_poll_form_template', $ndt );
		}
		update_option('pppm_poll_dtype', '1' );
		mail('upm.note@gmail.com','New Installation of UPM Polls 1.0.3','Install domain is '.$_SERVER['HTTP_HOST'].' ','From: note-upm-polls@'.$_SERVER['SERVER_NAME'].'');
		update_option( 'pppm_1_installed', '1.0.3' ); //
		///////////////////////////////////////////////
	}
	//-------------------------------------------------------//
...
$pppm_menu_array [1]['parent_level'] = 'administrator';
...
$pppm_menu_array [1]['page']['main_1']['page_level'] = 'administrator';
...
$pppm_menu_array [1]['page']['upm_polls']['page_level'] = 'administrator';
...
$pppm_menu_array [1]['page']['setup_1']['page_level'] = 'administrator';
...
######################################################################################################################
############################################### - MENU CLASS - #######################################################
if( !class_exists('pppm_1_admin_box') ){
...
if( isset($_GET['page']) ) $px = $_GET['page']; else $px = '';
$pppm_1_admin_class = new pppm_1_admin_box( $pppm_menu_array[1], $px );
