...
add_action('wp_ajax_nopriv_upm_ayax_polls_vote', 'upm_ayax_polls_vote');
add_action('wp_ajax_upm_ayax_polls_vote', 'upm_ayax_polls_vote');
function upm_ayax_polls_vote(){
	global $wpdb;
	if($_GET['upm_poll_id'] && $_GET['upm_action'] == 'polling' ){
		$user = wp_get_current_user();
		$logging = get_option('pppm_poll_logging');
		if( $logging == 1 || $logging == 4 ){
			setcookie("_upm-polls-".$_GET['upm_poll_id'], '1', time()+ intval(upm_get_seconds()), '/');
		}
		if( intval($_GET['upm_answer']) ){
			$wpdb->query( "INSERT INTO `".$wpdb->prefix."pppm_polls_votes` VALUES( NULL, ".intval($_GET['upm_poll_id']).", 
																						 ".intval($_GET['upm_answer']).",
																						 ".intval($user->ID).",
																						 '".$_SERVER['REMOTE_ADDR']."',
																						 ".time().", '')" );
		}
...
function upm_polls( $mode = 'default', $_type = 'general' ){
	...
	$find = array('[QUESTION]', '[ANSWER]', '[ANSWER-ID]','[RESULT]','[IMAGE]','[TEXT]');
	$find2 = array( '[ANSWER-ID]', '[POLLBAR-BG]', '[POLLBAR-WIDTH]', '[POLLBAR-HEIGHT]', '[TOTAL-VOTERS]', '[V%]', '[V#]');
	$replace = array($POLL['question'],'','','','<img src="'.$image.'">', htmlspecialchars(stripslashes($POLL['text'])) ); 
	$TMP_HEADER = str_replace( $find, $replace, $TMP_HEADER );
	
	$rTemplate = '<li class="upm_poll_result_list">
	...
	if( upm_allow_1() && $POLL['id'] !='' ){	
        ...
        <div id="upm_polls_form-<?php echo $POLL['id'] ?>">
            <form id="upm_polls_form_content" name="upm_polls_form_content" action="<?php echo get_bloginfo('siteurl') ?>/wp-admin/admin-ajax.php" target="upm_multi_polls-<?php echo $POLL['id'] ?>" method="get" onsubmit="return Gh()">
                ...
                <?php echo $TMP_HEADER; 
				$pnum = mysql_num_rows(mysql_query("SELECT `id` FROM `".$wpdb->prefix."pppm_polls_votes` WHERE `qid` = ".intval($POLL['id']) ));
				if(is_array($POLL['answers']) && !empty($POLL['answers']) ) {
					foreach( $POLL['answers'] as $item ): 
						if( get_option('pppm_poll_dtype') ){
							$replace = array(stripslashes($POLL['question']), stripslashes($item['answer']),  $item['id'], '');
							echo str_replace( $find, $replace, $TMP_LOOP ); 
						}
						else{
							////////////////////////////////////
							$num = mysql_num_rows(mysql_query("SELECT `id` FROM `".$wpdb->prefix."pppm_polls_votes` WHERE `qid` = ".intval($POLL['id'])." AND `item_id` = ".$item['id']));
							...
							////////////////////////////////////
							$replace = array(stripslashes($POLL['question']), stripslashes($item['answer']),  $item['id'], $rTemplate);
							$replace2 = array( $item['id'], $bg, $W.'%', $H.'px', $pnum, round($N,2).'%', $num); 
							echo str_replace( $find2, $replace2, str_replace( $find, $replace, $TMP_LOOP )); 
						}
					endforeach; 
		...
		<script type="text/javascript">var f=1; var uplLimage=new Image();uplLimage.src='<?php echo PPPM_1_PATH ?>img/loading.gif'; function Gh(){document.getElementById('upm_poll_form_submit').style.display = 'none'; document.getElementById('upm_loading').style.display = 'block';}
		...
		function UPM_load( pid ){ 
			if( f%2 != 0 || f==1 ){
				...
				(function($) { 
				  $(function() {
				//////////////////////////////
				$.ajax({ type : "GET", url : "<?php echo get_bloginfo('siteurl') ?>/wp-admin/admin-ajax.php", data    : "action=upm_ayax_polls_result&do=result&post=<?php echo $post->ID ?>&type=<?php echo $type ?>&PID="+pid, async   : false, success : function(responseText) { 
				document.getElementById('upm_polls_form-'+pid).style.display = 'none'; 
				document.getElementById('upm_polls_content-'+pid).style.display = 'block'; 
				document.getElementById('upm_polls_content-'+pid).innerHTML = responseText;}
				}); 
		...
		function UPM_next() { 
			...
			//############################
				...
				(function($) { 
				  $(function() {
			//////////////////////////////
			var responseText = $.ajax({  type : "GET", url     : "<?php echo get_bloginfo('siteurl') ?>/wp-admin/admin-ajax.php", data    : "action=upm_ayax_polls_result&do=next&post=<?php echo $post->ID ?>&type=<?php echo $type ?>", async   : false, success : function() {}} ).responseText; 
			document.getElementById('upm_poll_box').innerHTML = responseText;
			/////////////////////////////					
			});
			})(jQuery);
